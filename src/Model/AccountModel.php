...
class AccountModel extends Database
{
    public function createAccount($user_id, $amount = 0){
        return $this->create("INSERT INTO `accounts` (`user_id`, `balance`) VALUES
(?, ?)","id", [$user_id, $amount]);
    }
    ...
    public function getAccounById($user_id){
        return $this->select("SELECT * FROM `accounts` WHERE  `user_id` = ? LIMIT 1","i", [$user_id]);
    }
    ...
    public function getBalanaceById( $account_id){
        return $this->select("SELECT `balance` as `Balance` FROM `accounts` WHERE`id` = ? LIMIT 1","i", [$account_id]);
    }
